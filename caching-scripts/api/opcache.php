    ...
    case 'reset':
        if (function_exists('opcache_reset')) {
            if (opcache_reset()) {
                ...
                // Log the action
                $log_entry = date('Y-m-d H:i:s') . " - Admin: " . $_SESSION['cache_admin_username'] . " - OPcache reset\n";
    ...
    case 'invalidate':
        ...
        if (empty($file)) {
            ...
            if (opcache_invalidate($file, true)) {
                ...
                // Log the action
                $log_entry = date('Y-m-d H:i:s') . " - Admin: " . $_SESSION['cache_admin_username'] . " - OPcache invalidate: $file\n";
                file_put_contents('/var/log/caching-scripts/activity.log', $log_entry, FILE_APPEND | LOCK_EX);
            } else {
    ...
    case 'compile':
        ...
        if (empty($file)) {
            ...
            if (file_exists($file) && opcache_compile_file($file)) {
                ...
                // Log the action
                $log_entry = date('Y-m-d H:i:s') . " - Admin: " . $_SESSION['cache_admin_username'] . " - OPcache compile: $file\n";
                file_put_contents('/var/log/caching-scripts/activity.log', $log_entry, FILE_APPEND | LOCK_EX);
            } else {
