    ...
    case 'flushall':
        try {
            if ($redis->flushAll()) {
                ...
                // Log the action
                $log_entry = date('Y-m-d H:i:s') . " - Admin: " . $_SESSION['cache_admin_username'] . " - Redis flushall\n";
    ...
    case 'flushdb':
        ...
        try {
            ...
            if ($redis->flushDB()) {
                ...
                // Log the action
                $log_entry = date('Y-m-d H:i:s') . " - Admin: " . $_SESSION['cache_admin_username'] . " - Redis flushdb: $db\n";
                file_put_contents('/var/log/caching-scripts/activity.log', $log_entry, FILE_APPEND | LOCK_EX);
            } else {
        ...
        if (empty($key)) {
            ...
            try {
                ...
                if ($redis->del($key)) {
                    ...
                    // Log the action
                    $log_entry = date('Y-m-d H:i:s') . " - Admin: " . $_SESSION['cache_admin_username'] . " - Redis delete key: $key (db: $db)\n";
                    file_put_contents('/var/log/caching-scripts/activity.log', $log_entry, FILE_APPEND | LOCK_EX);
                } else {
        ...
        if (empty($key)) {
            ...
            try {
                ...
                if ($result) {
                    ...
                    // Log the action
                    $log_entry = date('Y-m-d H:i:s') . " - Admin: " . $_SESSION['cache_admin_username'] . " - Redis set key: $key (db: $db)$ttl_msg\n";
                    file_put_contents('/var/log/caching-scripts/activity.log', $log_entry, FILE_APPEND | LOCK_EX);
                } else {
