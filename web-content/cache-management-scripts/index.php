...
function isLoggedIn() {
    return isset($_SESSION['admin_logged_in']) && 
           ...
           (time() - $_SESSION['admin_login_time']) < $GLOBALS['admin_config']['session_timeout'];
}
...
if (!isLoggedIn()) {
    ...
    <head>
        ...
        <title>Cache Management - Admin Login</title>
        <style>
            * {
                margin: 0;
                padding: 0;
...
if ($_POST && isset($_POST['action']) && $_POST['action'] === 'execute_script') {
    ...
    if (!in_array($script, $allowed_scripts)) {
        ...
        // Log the action
        $log_entry = date('Y-m-d H:i:s') . " - Admin: " . $_SESSION['admin_username'] . " - Executed: $command\n";
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Cache Management Scripts - Admin Panel</title>
    <style>
        * {
            margin: 0;
            padding: 0;
    ...
    <div class="container">
        <div class="header">
            <div class="header-content">
                <h1>🛠️ Cache Management Scripts</h1>
                <p>Admin Panel for System Cache Management</p>
            </div>
            <div class="admin-info">
                <div class="admin-badge">👤 Admin: <?php echo htmlspecialchars($_SESSION['admin_username']); ?></div>
                <div>
        ...
        <div class="warning">
            ⚠️ <strong>Warning:</strong> These scripts have system-level access. Use with caution and ensure you understand the impact of each command before execution.
        </div>
