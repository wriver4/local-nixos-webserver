...
// Function to find current NixOS hosts file
function findNixOSHostsFile() {
    $hosts_files = glob('/nix/store/*-hosts');
    if (empty($hosts_files)) {
        return '/etc/hosts'; // Fallback
    }
    
    // Find the most recent or active hosts file
    foreach ($hosts_files as $hosts_file) {
        if (is_readable($hosts_file) && strpos(file_get_contents($hosts_file), '127.0.0.1') !== false) {
            return $hosts_file;
        }
    }
    
    return $hosts_files[0]; // Return first if none found with localhost
}

// Function to update NixOS hosts configuration
function updateNixOSHosts($domain, $action = 'add') {
    $webserver_config = '/etc/nixos/webserver.nix';
    
    if (!file_exists($webserver_config)) {
        return ['success' => false, 'message' => 'webserver.nix not found'];
    }
    
    $config_content = file_get_contents($webserver_config);
    $backup_file = $webserver_config . '.backup.' . date('Y-m-d-H-i-s');
    ...
    if ($action === 'add') {
        ...
        if (strpos($config_content, '"' . $domain . '"') !== false) {
            return ['success' => false, 'message' => 'Domain already exists in NixOS configuration'];
        }
        
        // Add domain to networking.hosts
        $pattern = '/("127\.0\.0\.1" = \[\s*)/';
        $replacement = '$1' . "\n      \"$domain\"";
        $new_content = preg_replace($pattern, $replacement, $config_content);
        
        if ($new_content === null || $new_content === $config_content) {
            return ['success' => false, 'message' => 'Failed to add domain to NixOS configuration'];
        }
        ...
        if ($new_content === null) {
            return ['success' => false, 'message' => 'Failed to remove domain from NixOS configuration'];
        }
    }
    
    // Write updated configuration
    if (file_put_contents($webserver_config, $new_content) === false) {
        return ['success' => false, 'message' => 'Failed to write NixOS configuration'];
    }
    
    return [
        'success' => true, 
        'message' => 'NixOS configuration updated successfully',
        'backup' => $backup_file,
        'requires_rebuild' => true
    ];
}

// Function to generate nginx virtual host configuration for NixOS
function generateNixOSVirtualHost($domain, $document_root = '/var/www', $ssl_enabled = false) {
    $config = "      # {$domain}\n";
    $config .= "      \"{$domain}\" = {\n";
    ...
    $config .= "              fastcgi_param PHP_VALUE \"auto_prepend_file=none\";\n";
...
// Function to update NixOS virtual hosts configuration
function updateNixOSVirtualHosts($sites) {
    $webserver_config = '/etc/nixos/webserver.nix';
    $backup_file = $webserver_config . '.backup.' . date('Y-m-d-H-i-s');
    ...
    $current_config = file_get_contents($webserver_config);
    ...
    foreach ($sites as $site) {
        if ($site['status'] === 'active') {
            $virtual_hosts .= generateNixOSVirtualHost(
                $site['domain'], 
                $site['document_root'], 
                $site['ssl_enabled']
            );
    ...
    if ($new_config === null) {
        return ['success' => false, 'message' => 'Failed to update NixOS configuration'];
    }
    
    // Write updated configuration
    if (file_put_contents($webserver_config, $new_config) === false) {
        return ['success' => false, 'message' => 'Failed to write NixOS configuration file'];
    }
        ...
        switch ($_POST['action']) {
            case 'toggle_status':
                if (isset($_POST['site_id'])) {
                    ...
                    $message = 'Site status updated successfully. Rebuild NixOS configuration to apply changes.';
                ...
                if (empty($name) || empty($domain)) {
                    ...
                    try {
                        ...
                        if ($check_stmt->fetchColumn() > 0) {
                            ...
                            // Add to NixOS hosts configuration
                            $hosts_result = updateNixOSHosts($domain, 'add');
                            ...
                            if (empty($message)) {
                                ...
                                if ($hosts_result['success']) {
                                    $message .= 'Domain added to NixOS configuration. ';
                                    $requires_rebuild = true;
                                } else {
                                    $message .= 'Warning: ' . $hosts_result['message'] . ' ';
                                }
                                $message .= 'Remember to rebuild NixOS configuration.';
                                $message_type = 'success';
                            }
                        }
                    } catch (Exception $e) {
                    ...
                    if ($site) {
                        ...
                        if (in_array($site['domain'], $protected_domains)) {
                            ...
                            try {
                                ...
                                // Remove from NixOS hosts configuration
                                $hosts_result = updateNixOSHosts($site['domain'], 'remove');
                                ...
                                if ($hosts_result['success']) {
                                    $message .= 'Domain removed from NixOS configuration. ';
                                    $requires_rebuild = true;
                                } else {
                                    $message .= 'Warning: ' . $hosts_result['message'] . ' ';
                                }
                                $message .= 'Remember to rebuild NixOS configuration and manually remove the directory if needed.';
                                $message_type = 'success';
                            } catch (Exception $e) {
            ...
            case 'rebuild_config':
                ...
                $result = updateNixOSVirtualHosts($sites);
                
                if ($result['success']) {
                    $log_stmt = $pdo->prepare("INSERT INTO logs (site_id, action, details) VALUES (NULL, 'Config rebuilt', 'NixOS configuration updated')");
                    $log_stmt->execute();
                    
                    $message = 'NixOS configuration updated! Run "sudo nixos-rebuild switch" to apply changes.';
                    $message_type = 'success';
                    $requires_rebuild = true;
                } else {
...
// Get current hosts file info
$current_hosts_file = findNixOSHostsFile();
$hosts_file_readable = is_readable($current_hosts_file);
?>
...
<head>
    ...
    <title>Web Server Dashboard - NixOS Virtual Host Management</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <div class="header">
            <h1>🚀 NixOS Web Server Dashboard</h1>
            <p>Manage your NixOS web server with nginx, PHP 8.4, MySQL and Virtual Host Management</p>
        </div>
        ...
        <?php if ($requires_rebuild): ?>
            <div class="rebuild-notice">
                🔄 <strong>NixOS Rebuild Required:</strong> Run <code>sudo nixos-rebuild switch</code> to apply configuration changes.
            </div>
        <?php endif; ?>
        ...
        <div class="grid">
            <div class="card">
                ...
                <div class="hosts-info">
                    <h4>📍 NixOS Hosts File Information</h4>
                    <p><strong>Current hosts file:</strong></p>
                    <div class="hosts-path"><?php echo htmlspecialchars($current_hosts_file); ?></div>
                    <p style="margin-top: 10px;">
                        <strong>Status:</strong> 
                        <?php if ($hosts_file_readable): ?>
                            <span style="color: #22543d;">✅ Readable</span>
                        <?php else: ?>
                            <span style="color: #742a2a;">❌ Not readable</span>
                        <?php endif; ?>
                    </p>
                    <p style="margin-top: 10px; font-size: 0.8rem; color: #718096;">
                        <strong>Note:</strong> In NixOS, hosts entries are managed via <code>networking.hosts</code> in configuration files. 
                        Changes require <code>nixos-rebuild switch</code> to take effect.
                    </p>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <form method="post">
                    ...
                    <p style="margin-top: 10px; font-size: 0.9rem; color: #718096;">
                        <strong>Note:</strong> Adding a site will automatically update the NixOS hosts configuration. 
                        A rebuild will be required to activate the changes.
                    </p>
                </form>
            </div>
    ...
    <div id="removeModal" class="modal">
        <div class="modal-content">
            ...
            <p style="color: #744210; margin-top: 10px;">The domain will also be removed from the NixOS hosts configuration.</p>
